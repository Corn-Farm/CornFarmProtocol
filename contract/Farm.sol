...
contract MasterChef is Ownable {
    ...
    // Heco Mainnet
    address public constant UNIV2ROUTER2 = 0xED7d5F38C79115ca12fe6C0041abb22F0A06C300;
    ...
    function setPair(address _pairaddr) public onlyOwner {
        ...
        IERC20(WHT).safeApprove(UNIV2ROUTER2, uint(-1));
        IERC20(HUSD).safeApprove(UNIV2ROUTER2, uint(-1));
        IERC20(USDT).safeApprove(UNIV2ROUTER2, uint(-1));
    ...
    function deposit(uint256 _pid, uint256 _amount) public payable {
        ...
        if(_amount > 0) {
            if (pool.poolType == POOL_TOKEN) {
                liquidity = tokenToLp(pool.lpToken, pool.moneyToken, _amount.sub(_amount.mul(1).div(100)));
            } else if (pool.poolType == POOL_LPTOKEN) {
                liquidity = lptokenToLp(pool.lpToken, pool.moneyToken, _amount.sub(_amount.mul(1).div(100)));
            }
    ...
    function swapExactTokensForTokens(address _fromToken, address _toToken, uint256 _amount) internal returns (uint256) {
        if (_fromToken == _toToken || _amount == 0) return _amount;
        return UniTools.swapExactTokensForTokens(UNIV2ROUTER2, _fromToken, _toToken, _amount);
    }

    function tokenToLp(IERC20 _token, address _moneyToken, uint256 _amount) internal returns (uint256 liq) {
        if (_amount == 0) return 0;

        if (address(_token) != USDT) {
            _amount = UniTools.swapExactTokensForTokens3(UNIV2ROUTER2, address(_token), _moneyToken, USDT, _amount);
        }

        // slippage 0.5% , ETH: 0.5*(1+0.005) = 0.5025
        uint256 amountBuy = _amount.mul(5025).div(10000);
        uint256 amountEth = _amount.sub(amountBuy);
        uint256 amountToken = swapExactTokensForTokens(USDT, address(sushi), amountBuy);

        (,, liq) = IUniswapV2Router02(UNIV2ROUTER2).addLiquidity(
                USDT, address(sushi), amountEth, amountToken, 0, 0, address(this), 0xffffffff);
    }

    function lptokenToLp(IERC20 _lpToken, address _midToken, uint256 _amount) internal returns (uint256 liq) {
        if (_amount == 0) return 0;
        address token0 = IUniswapV2Pair(address(_lpToken)).token0();
        address token1 = IUniswapV2Pair(address(_lpToken)).token1();
		...
		if (token0 == USDT || token1 == USDT) {
            amount0 = swapExactTokensForTokens(token0, USDT, amount0);
            amount1 = swapExactTokensForTokens(token1, USDT, amount1);
            return tokenToLp(IERC20(USDT), address(0), amount0.add(amount1));
		} else if (token0 == WHT || token1 == WHT) {
            amount0 = swapExactTokensForTokens(token0, WHT, amount0);
            amount1 = swapExactTokensForTokens(token1, WHT, amount1);
            return tokenToLp(IERC20(WHT), address(0), amount0.add(amount1));
		} else if (token0 == HUSD || token1 == HUSD) {
            amount0 = swapExactTokensForTokens(token0, HUSD, amount0);
            amount1 = swapExactTokensForTokens(token1, HUSD, amount1);
            return tokenToLp(IERC20(HUSD), address(0), amount0.add(amount1));
		} else {
            amount0 = swapExactTokensForTokens(token0, _midToken, amount0);
            amount1 = swapExactTokensForTokens(token1, _midToken, amount1);

            amount0 = swapExactTokensForTokens(_midToken, WHT, amount0.add(amount1));
            return tokenToLp(IERC20(WHT), address(0), amount0);
		}
    }
    function withdrawEth(address _to, uint256 _amount, bool _isWeth) internal {
        if (_isWeth) {
            IERC20(WHT).safeTransfer(_to, _amount);
        } else {
		    IERC20(WHT).safeTransfer(address(wethelper), _amount);
		    wethelper.withdraw(WHT, _to, _amount);
        }
    }
